...
public class HorseRacing {
    ...
    public void settleBets() {
        for (Bet bet : bets) {
            ...
            if (payout > 0) {
                System.out.println("Вы выиграли " + payout*2 + " сом");
            } else {
    ...
    public static void main(String[] args) {
        ...
        // добавляем лошадей
        horseRacing.addHorse(new Horse("Бойка"));
        horseRacing.addHorse(new Horse("Ахиллес"));
        horseRacing.addHorse(new Horse("Аргымак"));
        horseRacing.addHorse(new Horse("Ак-Кула"));
        horseRacing.addHorse(new Horse("Вулкан"));
        horseRacing.addHorse(new Horse("Барон"));
        horseRacing.addHorse(new Horse("Пегас"));
        horseRacing.addHorse(new Horse("Шторм"));
        horseRacing.addHorse(new Horse("Алай"));
        horseRacing.addHorse(new Horse("Арбат"));
        Scanner scanner = new Scanner(System.in);
        boolean quit = false;

        while (!quit) {
            System.out.println("Введите 1, чтобы сделать ставку. Введите 2, чтобы начать гонку. Введите 3, чтобы выйти:");
